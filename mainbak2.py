...
def index():
  ...
  if request.method == 'GET':
    cloneguys=[]
    if 'donor' in session:
      #set guys to all donors, set session['donor'] to... itself IF it's present in donors
      #cloneguys stays cloneguys unless guys is empty, I think.  don't remember why it works
      #this way
      (guys, session['donor'], cloneguys)=initialize(session['donor'], "")
      if 'searchval' not in session or 'blanco' not in session:
        #this is hacky, really need to refactor everything
        reset()
    else:
      #initial page load
      ...
      #reset searchval, blanco for same reason
    ...
    #wait why am i doing this exact same line twice
    #(guys, session['donor'], cloneguys)=initialize(session['donor'], "")
    
    if request.args.get('home') == 'home':
      reset()
      (guys, session['donor'], cloneguys)=initialize("", "")
    if request.args.get('home') == 'search':
      reset()
    ...
    elif request.args.get('home') == 'blanco':
      reset()
      
      session['blanco'] = request.args.get('searchval')
    ...
    if request.args.get('other_folder') != None:
      session['current_folder']=request.args.get('other_folder')
      #make sure they're not messing with the URL
      if not os.path.isdir(os.path.join("static","sinks",session['donor'],session['current_folder'])):
        del session['donor']
        del session['current_folder']
        flash("quit messing with the URL")
        return redirect('/')
                             
    elif request.args.get('donor') != None:
      session['donor']=request.args.get('donor')
      
      if not session['donor'] or not os.path.isdir(os.path.join("static","sinks",session['donor'])):
        del session['donor']
        del session['current_folder']
        flash("quit messing with the URL")
        return redirect('/')
      else:
        session['donor']=Donator.query.filter_by(location=session['donor']).first().location
        
    if request.args.get('home') =='chat':
...
####################################################################################################
    ...
    if request.args.get('home') == 'search' or request.args.get('home') == 'blanco':
  ...
  if not session['current_folder'] or (request.args.get('donor')!=None or
                                       request.args.get('home') == 'search' or
                                       request.args.get('home') == 'blanco' or
                                       request.args.get('home') == 'home'):
  ...
  return render_template('silicate.html',title="silicatewastes", guys=guys,
                         ...
                         display_current=session['current_folder'].upper(),
                         username=username,
...
######################/sink/sink/sink/sink/sink/sink/sink/sink/sink/sink/sink###############
######################/sink/sink/sink/sink/sink/sink/sink/sink/sink/sink/sink###############
####BY THE GODS I NEED TO REFACTOR THIS WITH A CAPITAL R####################33
...
def sink():
  ...
  #3 sink (itself) page sent us here by clicking username
  
  #case 1 & 2 we come to this page, load the cad variable
  #case 3 we're leaving this page to stalkmore
  if request.method == 'GET' :
    #case 1 or 2, get the cad variable
    ...
    #case 3 get the member variable
    if request.args.get('member') != None:
      #if it's not an int, let it crash
      member = int(request.args.get('member'))
      #don't let them go to stalkmore if not logged in
      if 'username' not in session:
        flash("log in")
      else:
        username=session['username']
        #get the member's row
        memberdata=User.query.filter_by(id=member).first()
        #grab sink.location and all? rating info from every sink he's rated
  ...
  username=""
  #goes with current sink in URL
  new_rating=""
  #last sink downloaded
  previous_sink=""
  #goes with previous_sink
  previous_rating=""
  
  #username again.  think i need to reorganize this
  if 'username' in session:
    username=session['username']
    user = User.query.filter_by(username=username).first()
    
    #clear out lastsink, stop bothering user about it
    #checking for POST again?  what a mess.  need to rethink this
    if request.method=='POST' and 'forgot' in request.form:
      user.lastsink=0
      db.session.commit()
    #so i guess this needs to happen get or post
    if user.lastsink!=0 and user.lastsink != cad:
      ...
      #(if previous_sink is empty string that's the switch sink.html uses)
      if previous_rating!=None:
        previous_sink=""
        
      #but this only on post.  this is spagetti code
      #he clicked add rating on previous sink
      elif request.method=='POST' and 'rate_previous' in request.form:
        if 'starsB' not in request.form:
          ...
          #stick in the new rating, stop bothering him about it
            ...
            #i think previous_rating is just means previous sink's rating?
            #even though this is a "new" rating
...
def downloadfile():
  #sinsplit called from /sink sets session['donor']
  #spagetti code
